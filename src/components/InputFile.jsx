...
export const InputFile = ({ label, required, value, onChange, accept = "image/*", disabled = false, error }) => {
    const [isDragging, setIsDragging] = useState(false);
    const inputRef = useRef(null);

    const handleDrag = (e) => { e.preventDefault(); setIsDragging(e.type === "dragenter" || e.type === "dragover"); };
    const handleDrop = (e) => { e.preventDefault(); setIsDragging(false); const file = e.dataTransfer.files?.[0]; if (file) onChange?.({ target: { files: [file] } }); };
            ...
            {value ? (
                <div className="relative w-full h-40 border border-[var(--border)] rounded-[var(--radius-sm)] overflow-hidden group">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black/45 text-white text-xs p-2 truncate opacity-0 group-hover:opacity-100 transition-opacity">
                        {typeof value === "string" ? value.split("/").pop() : "Imagem carregada"}
                    </div>
                </div>
            ) : (
                <div onDragEnter={handleDrag} onDragOver={handleDrag} onDragLeave={handleDrag} onDrop={handleDrop}
                    ...
                    <p className="font-medium text-[var(--text-secondary)] mb-1 text-sm">Arraste e solte ou clique para selecionar</p>
                    <p className="text-xs text-[var(--text-muted)] mb-3">PNG, JPG, WEBP até 5MB</p>
                    <button type="button" className="btn-secondary px-4 py-1.5 text-sm rounded-[var(--radius-sm)]" onClick={e => e.stopPropagation()}>
                        Escolher arquivo
                    </button>
                </div>
            )}
            <input ref={inputRef} type="file" accept={accept} onChange={onChange} disabled={disabled} className="hidden" />
