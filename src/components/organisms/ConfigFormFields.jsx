...
export const ConfigFormFields = ({ colaborador, setColaborador, credencial, setCredencial }) => {
  return (
    <div className="grid gap-4">
      <div>
        <label className="text-xs uppercase tracking-wide text-[var(--text-muted)]">Nome</label>
        <input
      ...
      <div>
        <label className="text-xs uppercase tracking-wide text-[var(--text-muted)]">Telefone</label>
        <input
          className="form-input mt-2"
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => setCredencial({ ...credencial, email: e.target.value })}
        />
      </div>
    </div>
  );
