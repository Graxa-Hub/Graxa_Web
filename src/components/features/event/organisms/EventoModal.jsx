...
import { EnderecoForm } from "../../../organisms/EnderecoForm";
import { BandaTurneSelectorForm } from "../../../ModalEventos/BandaTurneSelectorForm";
...
export function EventoModal({
  ...
  const validateAll = () => {
    ...
    if (activeTab === "show") {
      ...
      if (showData.turneId && showData.dataHoraInicio && showData.dataHoraFim) {
        const limitesErro = verificarLimitesTurne(
          showData.turneId,
          ...
          setFieldErrors({
            general: (
              <div>
                ...
                <div className="text-sm bg-[var(--surface)] p-3 rounded border border-[var(--border)]">
                  O evento deve ocorrer entre{" "}
                  <strong>{limitesErro.turneInicio}</strong> e{" "}
                  <strong>{limitesErro.turneFim}</strong>.
                </div>
              </div>
            ),
          });
  ...
  return (
    ...
    >
      <div
        style={{ position: "fixed", inset: 0, background: "var(--overlay)" }}
        onClick={onClose}
      ></div>
      <div className="relative w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto rounded-[var(--radius-md)] shadow-[var(--shadow-card)]">
        ...
        <div className="p-6 bg-[var(--surface-elevated)]">
          ...
          {fieldErrors.general && (
            <div
              className="mb-4 p-4 rounded-[var(--radius-sm)]"
              style={{
