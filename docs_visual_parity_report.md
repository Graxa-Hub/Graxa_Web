...
- Refino adicional de modais principais (`ArtistaModal`, `TurneModal`, `BandaTurneSelector`, notificações e caixas auxiliares).
- Refatoração visual de páginas e blocos compartilhados como recuperação de senha, calendário, configuração do usuário e **Visão de Evento**.
- Migração dos componentes internos da Visão de Evento (`AgendaList`, `AgendaCard`, `PainelDireito`, `MapCard`, `ClimaCard`, `Porcentagem`, `AnotacoesCard`, `DiaInfoCard`).
